...
LICENSE="GPL-2 BSD"
SLOT="0"
IUSE="+X avif brotli +client +clipboard crypt csc cuda cups dbus debug doc examples evdi gstreamer +gtk3 gui html ibus jpeg +libyuv +lz4 lzo mdns minimal +nvenc +nvfbc oauth opengl openh264 pinentry pulseaudio pyglet qrcode qt6 quic +server sound spng systemd test tk +trayicon udev vpx webcam webp x264 xdg xinerama "
...
src_prepare() {
	...
	cp \
		/usr/local/lib64/pkgconfig/{libyuv,evdi}.pc \
	"${S}/fs/lib/pkgconfig/" || die
}
...
python_configure_all() {
	...
	DISTUTILS_ARGS=(
		...
		"$(use_with libyuv csc_libyuv)" # https://chromium.googlesource.com/libyuv/libyuv
		...
		"$(use_with nvfbc nvfbc)" # NVIDIA Capture SDK
		"$(use_with opengl)"
		"$(use_with openh264)"
	...
	# we ship these with nvidia-cuda-toolkit
	rm "${S}/fs/lib/pkgconfig/"{cuda,nvjpeg}".pc" || die
...
python_test() {
	die "https://github.com/Xpra-org/xpra/commit/c704f1962b4b66e2e9a5995d42abd391d1f6a9ac"
