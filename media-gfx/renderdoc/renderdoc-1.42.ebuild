...
src_prepare() {
	rm -r qrenderdoc/3rdparty/catch
	# rm -r qrenderdoc/3rdparty/flowlayout
	rm -r qrenderdoc/3rdparty/pyside
	rm -r qrenderdoc/3rdparty/python
	# rm -r qrenderdoc/3rdparty/pythoncapi_compat.h
	rm -r qrenderdoc/3rdparty/qt
	# rm -r qrenderdoc/3rdparty/scintilla
	rm -r qrenderdoc/3rdparty/swig
	# rm -r qrenderdoc/3rdparty/toolwindowmanager
	...
	# 	|| die 'sed QT += qt5compat failed'

	sed \
		-e 's/QT += x11extras/QT += core5compat/g' \
		-i \
			qrenderdoc/qrenderdoc.pro \
		|| die 'sed QT += core5compat failed'
}
