...
IUSE="
	...
	debug doc +draco +embree +ffmpeg +fftw +fluid +gmp gnome hip hiprt hydra jack
	jemalloc jpeg2k man manifold materialx +nanovdb ndof nls +oidn oneapi openal +openexr +opengl +openpgl
	+opensubdiv +openvdb optix osl pipewire +pdf +potrace +pugixml pulseaudio
	renderdoc sdl +sndfile +tbb test +tiff +truetype usd valgrind vulkan wayland +webp X
"
...
src_prepare() {
	...
	if use optix; then
		sed -e "s/compute_75/compute_89/g" -i intern/cycles/kernel/CMakeLists.txt || die
	fi
