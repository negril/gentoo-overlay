...
src_test() {
	...
	# 	--dump-config &> "${T}/FreeCADCmd_config.log"
	...
	if use gui; then
# 		local -x QT_QPA_PLATFORM="${QT_QPA_PLATFORM:-offscreen}"

# 		virtx "${BUILD_DIR}/bin/FreeCAD" --dump-config &> "${T}/FreeCAD_config.log"
		...
		# 	--dump-config &> "${T}/FreeCAD_config.log"
