...
# TODO
# EIGEN_TEST_CUDA_NVC
# AOCL MathLib / BLAS / LAPACK
# benchmarks are isolated projects
# demos is qt4 only
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-5.0.1-error-no-matching-function-for-call-to-get_test_prec.patch"
	"${FILESDIR}/${PN}-5.0.1-fix-TriangularSolverMatrix.patch"
...
src_configure() {
	...
	if use test; then
		...
		mycmakeargs+=( # {{{
			...
			# Use nvc++ (NVHPC) instead of nvcc to compile the CUDA tests
			# -DEIGEN_TEST_CUDA_NVC="$(usex cuda "$(usex cuda-nvc)")"
		...
		# # use !ducc       && mycmakeargs+=( -DDUCCFFT="FALSE" ) # DUCC_ROOT
		...
		# use !pocketfft  && mycmakeargs+=( -DPOCKETFFT="FALSE" )
...
src_test() {
	local CMAKE_SKIP_TESTS=(
		...
		"^basicstuff_8$" # Official

		"^matrix_power_8$" # Unsupported
		"^matrix_power_11$" # Unsupported
		"^matrix_square_root_3$" # Unsupported
	...
	if use cuda; then
		...
		CMAKE_SKIP_TESTS+=(
			...
			"^cxx11_tensor_gpu_9$" # Unsupported gpu
		)

		if use boost; then
			CMAKE_SKIP_TESTS+=(
				"^boostmultiprec_6$" # Official
			)
		fi
	fi
