...
cuda-toolkit_check_reqs() {
	# export CHECKREQS_DISK_BUILD="9351M"
	# export CHECKREQS_DISK_BUILD="7734M"
	if use amd64; then
		export CHECKREQS_DISK_BUILD="4913M"
	elif use arm64; then
		export CHECKREQS_DISK_BUILD="4851M"
	fi
...
cuda_verify() {
	if [[ ! -f "${DISTDIR}/${A}" ]]; then
		case ${EBUILD_PHASE} in
			setup) eqawarn "invalid A ${A}"; return ;;
			unpack|prepare) die "invalid A ${A}" ;;
			*) die "not supported in ${EBUILD_PHASE}"
		esac
	fi

	# # {{{ # BUG OVERLAY ONLY
	# since we don't extract the full archive anymore this isn't correct anymore
	# local unpacked_size
	# unpacked_size="$(sh "${DISTDIR}/${A}" --info | grep -oP "(?<=Uncompressed size: )[0-9]* [KMG]" | tr -d ' ')"
	# [[ -z "${unpacked_size}" ]] && die "could not extract unpacked_size"

	# local unit="${unpacked_size:0-1}"
	# case $unit in
	# 	K) unpacked_size="$(( ${unpacked_size:0:-1} / 1024 + 1 ))M";;
	# 	*) die "unknown unit ${unit}" ;;
	# esac

	# if [[ "$(echo -e "$CHECKREQS_DISK_BUILD\n$unpacked_size" | sort | tail -n 1)" != "$CHECKREQS_DISK_BUILD" ]]; then
	# 	die "CHECKREQS_DISK_BUILD ${CHECKREQS_DISK_BUILD} is to small, should be at least ${unpacked_size}"
	# fi
	# # }}}
...
pkg_pretend() {
	...
	# 	ewarn
	# 	ewarn "disabling installing static libs is untested and might lead to errors"
	# 	ewarn
	# fi
}
...
src_unpack() {
	...
	# if use static-libs; then
	# 	# We have to still extract libraries that only exist as static libs
	# 	local static_libs=(
	# 		builds/cuda_cuxxfilt/targets/x86_64-linux/lib/libcufilt.a
	# 		builds/cuda_nvcc/targets/x86_64-linux/lib/libnvptxcompiler_static.a
	# 		builds/cuda_sandbox_dev/targets/x86_64-linux/lib/stubs/libnvidia-sandboxutils_loader.a
	# 	)
	# 	printf "%s\n" "${static_libs[@]}"
	# 	eqawarn bash "${DISTDIR}/${A}" --tar xf "${static_libs[@]}"
	# 	bash "${DISTDIR}/${A}" --tar xf "${static_libs[@]}"
	# fi

	eqawarn "size:"
	du -sc "${WORKDIR}" | sort -n | while read -r size; do
		eqawarn "	${size}"
	done
}
...
src_install() {
	...
	# if ! use static-libs; then
	# 	sed -e "/\.a\"/d" -i.orig "${T}/install.sh" || die
	# fi
	...
	if use debugger; then
		if [[ -d "${ED}/${CUDA_PATH}/extras/Debugger/lib64" ]]; then
			eqawarn "${ED}/${CUDA_PATH}/extras/Debugger/lib64 exists"
	...
	eqawarn "sizes:"
	du -sc "${D}" "${S}" | sort -n | while read -r size; do
		eqawarn "	${size}"
	done
}
