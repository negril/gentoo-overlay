...
inherit check-reqs desktop toolchain-funcs unpacker python-r1
...
src_install() {
	...
	dofile() {
		...
		for file in ${1}; do
			...
			for f in "${fs[@]}"; do
				...
				chmod -R --reference="${f}" "${ED}${_DESTFILE}" || die "failed to copy permissions from ${_SRCFILE} to ${ED}${_DESTFILE}"
				eend $? # }}}
			done
		done
	}
	...
	dopcfile() {
		...
		cat > "${ED}${cudadir}/pkgconfig/${1}-${2}.pc" <<-EOF || die
			cudaroot=${cudadir}
	...
	if use nsight; then
		...
		sed -e "s#${nsys_dir_host}/nsight-sys#${nsys_dir_host}/nsys-ui#g" -i "${ED}/usr/share/applications/nsight-systems-"*.desktop || die
