...
DESCRIPTION="OOpenRGB Plugin that monitors your screen and triggers effects whenever a given rule is matched (colors, OCR, etc.)"
HOMEPAGE="https://gitlab.com/OpenRGBDevelopers/OpenRGBHardwareSyncPlugin"
LICENSE="GPL-2"

MY_PN="OpenRGBDisplayEventsPlugin"
...
src_prepare() {
	...
	rm -r Dependencies/leptonica || die
	ln -s "${ESYSROOT}/usr/include/leptonica" Dependencies/leptonica || die

	rm -r Dependencies/tesseract || die
	ln -s "${ESYSROOT}/usr/include/tesseract" Dependencies/tesseract || die
